...
**默认提示词：**
```
按格式输出一行：[镜头:宽景/中景/近景/特写] [光线:明亮/暗淡/逆光/柔和] [画质:★1-★5] 然后用30-50字描述主要内容。画质评分：★5=清晰稳定有层次，★3=普通可用，★1=模糊/抖动/无内容。
```

**需求驱动的提示词模板**（用户指定了任何 theme/mood/pacing/must_capture 时使用）：
```
根据剪辑目标（主题「<THEME>」氛围「<MOOD>」）按格式输出一行：[镜头:宽景/中景/近景/特写] [光线:明亮/暗淡/逆光/柔和] [画质:★1-★5] 然后用30-50字描述主要内容，突出与目标主题相关的内容。画质评分：★5=清晰稳定有层次，★3=普通可用，★1=模糊/抖动/无内容。
```
...
同时记录每个片段 seg_desc 中的 **[画质:★N]** 评分，供下一步分级使用。跳过 seg_desc 内容循环重复或完全空洞（< 5 字）的极端异常片段。

#### 2. 素材分级 Triage

基于 seg_desc 中的 **[画质:★N]** 标记与内容丰富度，为每个片段打等级：

| 等级 | 判断标准 | 用途 |
|------|---------|------|
| **A 级（精华）** | ★4-5，且内容有视觉冲击力或情感价值（动人风景/有感染力的人物瞬间/强烈运动感） | 优先用于开篇、高潮、结尾 |
| **B 级（可用）** | ★3，或 ★4 但内容较平淡 | 用于铺陈、过渡 |
| **C 级（跳过）** | ★1-2，或 seg_desc 描述循环重复/空洞无物 | 直接排除，不纳入选片 |

**强制规则**：
- 开篇第 1-2 个片段和结尾最后 1-2 个片段**必须从 A 级素材中选取**
- 高潮节拍（情感最强的 2-3 个片段）优先使用 A 级素材
- 若 A 级素材数量不足（< 5 个），放宽标准：将 ★3 中内容最丰富的片段升入 A 级，直到 A 级不少于 5 个
